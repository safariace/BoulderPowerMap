...
import { Box, Paper, Typography, FormControlLabel, Checkbox, Switch, Select, MenuItem, Slider, IconButton, Collapse, Tooltip, alpha } from '@mui/material';
...
export default function FilterBar() {
  ...
  return (
    <Box sx={{ position: 'absolute', bottom: 20, left: 20, zIndex: 10 }}>
      <Collapse in={expanded} unmountOnExit>
        <Paper elevation={3} sx={{
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5 }}>
            <Typography variant="h4" sx={{ color: palette.tealLight, m: 0 }}>Filters</Typography>
            <Tooltip title="Hide legend">
              <IconButton size="small" onClick={() => setExpanded(false)} sx={{ color: alpha(palette.parchment, 0.5), '&:hover': { color: palette.parchment } }}>
      ...
      {!expanded && (
        <Tooltip title="Show legend & filters" placement="right">
          <IconButton
            onClick={() => setExpanded(true)}
            sx={{
              bgcolor: alpha(palette.ink, 0.88),
              color: palette.parchment,
              backdropFilter: 'blur(12px)',
              border: `1px solid ${alpha(palette.parchment, 0.1)}`,
              '&:hover': { bgcolor: palette.ink },
            }}
          >
            <FilterList />
          </IconButton>
